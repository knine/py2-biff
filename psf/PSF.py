...
class BinaryFit:
	...
	def Run(self):
		...
		starttime = time.strftime('%A, %B %d, %Y %X %Z' ,time.localtime())      # Start Time

                os.system('nice -19 /usr/local/bin/BinaryFitPSF')			# Run The Binary Fitting Program

		endtime = time.strftime('%A, %B %d, %Y %X %Z' ,time.localtime())        # End Time

		LogFile = open("BinaryFitPSF.log", 'w')                              	# Open New Log File
		LogFile.write("\n" + "#" * 60 + "\n")
...
class Tiny:
        def __init__(self):
                self.ParameterFile = 'object.par'                                                       # Model PSF Parameter File

        def Tiny1(self):
		print "\n" + "#" * 60 + "\n"
		print "Running tiny1..."

                os.system('nice -19 /usr/local/tinytim/tiny1 ' + self.ParameterFile)                             # Run tiny1 in the current directory
        ...
        def Tiny2(self):
                ...
                os.system('nice -19 /usr/local/tinytim/tiny2 ' + self.ParameterFile)                             # Run tiny2 in the current directory
